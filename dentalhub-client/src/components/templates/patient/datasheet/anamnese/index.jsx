...
import anamneseForm from "@/form/resumed-anamnese.json";
...
const AnamnesePanel = ({
	data: { patientId, treatmentId: anamneseId },
	view = 4,
}) => {
	...
	React.useEffect(() => {
		...
		const getAnamnese = async () => {
			try {
				const response = await dental.form.anamnese.getById(anamneseId);

				if (response.patientId != patientId)
					return toast.warning("Anamnese inválida.");
		...
		getAnamnese();
	}, [handleUpdateTreatment, isNew, treatment, anamneseId, patientId]);

	if (anamneseId && !treatment && isLoaded) {
	...
	return (
		...
		>
			...
			>
				<CardHeader
					title={isNew ? "Nova Anamnese" : "Anamnese do Paciente"}
					sx={{
						paddingTop: "60px",
						width: "100%",
					...
					>
						<form
							onSubmit={async (event) => {
								...
								try {
									const response =
										await dental.form.anamnese.create({
											patientId,
									...
									router.push(
										`/aluno/painel/paciente/${patientId}/anamnese/${response.id}`,
									);
								} catch (error) {
									console.error(error);
								}
										...
										callback={async () => {
											if (!isValidating)
												return toast.warning(
													"Essa Anamnese não está disponível para validação.",
												);
											setIsModalOpen(true);
										}}
									>
			...
			{isModalOpen && !isTeacher && (
				<ModalParaValidacao
					state={modalState}
					list={answersList}
					data={{ patientId, studentId: user?.id, anamneseId }}
					targetName="anamnese"
