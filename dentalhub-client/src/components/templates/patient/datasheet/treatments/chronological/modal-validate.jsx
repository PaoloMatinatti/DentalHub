...
const ModalValitePlan = ({ state, data, setIsValidating }) => {
	...
	return (
		<div>
			...
			>
				...
				>
					<CardHeader
						title={"Enviar Plano para Validação"}
						sx={{
							paddingTop: "60px",
							width: "100%",
					...
					>
							...
							<Box>
								<Autocomplete
									...
									options={teacherList.map(
										(teacher, index) => ({
											label: `${teacher.name} (${teacher.registry})`,
											id: index,
										}),
									)}
						...
						>
							...
							<Button
								onClick={async () => {
									try {
										await dental.form.plans.setValidation({
											planId: data.planId,
...
export default ModalValitePlan;
